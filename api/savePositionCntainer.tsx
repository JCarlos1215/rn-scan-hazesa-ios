...
export const savePositionContainer = async ({
  coordenada,
  id,
  id_bl,
}: {
  ...
  try {
    ...
    if (response.data?.Error === 1) {
      ...
      Alert.alert(
        "Error",
        `No se pudo guardar la coordenada ${coordenada}: ${response.data.Mensaje}`
      );
    ...
    Alert.alert(
      "Error al guardar",
      `No se pudo guardar la posición: ${error.message || "Error desconocido"}`
    );
